...
# read operation data stored in the given files whose names are passed
# as argument.
#
# Usage:
# R --slave --args mrbench_filename testdfsio_read1_filename testdfsio_write1_filename testdfsio_read2_filename testdfsio_write2_filename image_filename < plot_table_read.r
# 
# Arguments:
...
read_size <- c(mrbench_data$X1, test_dfsio_read1_data$X1, test_dfsio_write1_data$X1, test_dfsio_read2_data$X1, test_dfsio_write2_data$X1, terasort_data$X1);
...
data <- data.frame(time, read_size, benchmark)
plot <- ggplot(data, aes(time, read_size)) 
...
# vertical axis
scale_y_continuous("Tamanho do read (em KB)") + 
facet_grid(. ~ benchmark, scales = "free_x") + opts(title = expression("Tamanho do read (em KB)"))
